...
import { DepartmentType } from '../../types/worklist';
import { 
...
interface WorklistPanelProps {
  ...
  onSelectPatient: (patientId: string, item: any) => void;
  onClose: () => void;
}
...
export const WorklistPanel: React.FC<WorklistPanelProps> = ({
  ...
  const { worklistItems, stats, isLoading, error, selectItem, selectedItem } = useWorklistStore();
      ...
      <div className="bg-white w-full max-w-2xl h-full overflow-hidden shadow-2xl flex flex-col">
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm text-gray-600">
                        <div>Age: <span className="font-medium">{item.patient.age}</span></div>
                        <div>Gender: <span className="font-medium capitalize">{item.patient.gender}</span></div>
                        ...
                        {department === 'scans' && item.scanType && (
                          <div className="col-span-2">
                            Scan: <span className="font-medium">{item.scanType} - {item.bodyPart}</span>
                          </div>
                        )}
        ...
        {worklistItems.length > 0 && (
          <div className="px-6 py-4 border-t border-gray-200 bg-gray-50 flex justify-between items-center">
            <button
              onClick={() => useWorklistStore.getState().refreshWorklist()}
