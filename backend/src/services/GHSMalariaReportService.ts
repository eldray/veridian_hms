...
export class GHSMalariaReportService {
  ...
  private static async fetchCommodityData(startDate: Date): Promise<any> {
    ...
    const commodityStocks = await prisma.malariaCommodityStock.findMany({
      where: {
        reportingMonth: {
  ...
  static async generateMalariaReport(startDate: Date, endDate: Date): Promise<MalariaReport> {
    ...
    for (const attendance of malariaAttendances) {
      ...
      for (const lab of attendance.LabTest) {
        ...
        const isPositive = lab.result && JSON.stringify(lab.result).toLowerCase().includes('positive');
  ...
  static exportToCSV(report: MalariaReport): string {
    const rows: string[] = [];
    ...
    const addRow = (name: string, data: any) => {
