// modules/referral/ReferralRoutes.ts
...
export function createReferralRoutes(prisma: PrismaClient): Router {  // ✅ FIXED - factory function
  const router = Router();
  ...
  const outgoingReferralValidation = [
    ...
    body('corporateAccountId').optional().isString().withMessage('Valid corporate account ID required')  // ✅ ADDED
  ];
  ...
  const statusUpdateValidation = [
    ...
    body('outcomeNotes').optional().isString().withMessage('Outcome notes must be a string')  // ✅ ADDED
  ];
