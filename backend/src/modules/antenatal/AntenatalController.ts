...
export class AntenatalController {
  ...
  createAntenatalBooking = [
    ...
    body('abortions').isInt({ min: 0 }).withMessage('Valid abortions required'),
    body('livingChildren').isInt({ min: 0 }).withMessage('Valid living children required'),
    body('lmp').isISO8601().withMessage('Valid LMP date required'),
    body('riskLevel').isIn(['low', 'moderate', 'high']).withMessage('Valid risk level required'),
    async (req: AuthRequest, res: Response) => {
      try {
        ...
        const data: CreateAntenatalBookingInput = req.body;
  ...
  updateAntenatalBooking = [
    ...
    body('abortions').optional().isInt({ min: 0 }),
    body('livingChildren').optional().isInt({ min: 0 }),
  ...
  createANCVisit = [
    ...
    body('gestationalAgeWeeks').isInt({ min: 0, max: 42 }).withMessage('Valid gestational age required'),
    body('weight').isFloat({ min: 0 }).withMessage('Valid weight required'),
    body('bloodPressure').isString().withMessage('Blood pressure required'),
    body('fetalHeartRate').isInt({ min: 60, max: 200 }).withMessage('Valid fetal heart rate required'),
    body('iptpGiven').isBoolean().withMessage('IPTP status required'),
    body('ttGiven').isBoolean().withMessage('TT status required'),
    body('dangerSignsPresent').isBoolean().withMessage('Danger signs status required'),
    async (req: AuthRequest, res: Response) => {
      try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
  ...
  createDeliveryRecord = [
    ...
    body('deliveryType').isIn(['spontaneous_vaginal', 'assisted_vaginal', 'cesarean_section']).withMessage('Valid delivery type required'),
    body('deliveryOutcome').isIn(['live_birth', 'stillbirth', 'miscarriage', 'ectopic']).withMessage('Valid delivery outcome required'),
    body('gestationalAgeWeeks').isInt({ min: 0, max: 42 }).withMessage('Valid gestational age required'),
    async (req: AuthRequest, res: Response) => {
      try {
        const errors = validationResult(req);
  ...
  createPostnatalRecord = [
    body('patientId').notEmpty().withMessage('Patient ID required'),
    body('registrationDate').isISO8601().withMessage('Valid registration date required'),
    body('maternalCondition').isIn(['stable', 'complicated']).withMessage('Valid maternal condition required'),
    body('newbornCondition').isIn(['stable', 'complicated']).withMessage('Valid newborn condition required'),
    body('breastfeedingStatus').isIn(['exclusive', 'mixed', 'formula', 'not_applicable']).withMessage('Valid breastfeeding status required'),
    async (req: AuthRequest, res: Response) => {
      try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
