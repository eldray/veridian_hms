...
// ✅ Only auto-run when executed directly (Windows-safe)
const isDirectRun = (() => {
  ...
  const modulePath = new URL(import.meta.url).pathname
    ...
    .replace(/\\/g, '/')
    .toLowerCase();
  return scriptPath.endsWith(modulePath.split('/').pop() ?? '__never__') &&
    scriptPath.includes(modulePath.split('/').slice(-2, -1)[0] ?? '__never__');
})();
